...
ccc <- teams_valid |> 
  ...
  write_csv(here::here("data", "teams_country@18.csv"))
...
```{r cumulative-plot-sss3, echo=FALSE, warning=FALSE, error=TRUE, message=FALSE}
#| fig-cap: !expr paste0("Cumulative count of team creation, 2025 in **", colorize('blue'), "**, 2024 in **", colorize("green"), "**.")
...
ggplot() +
  ...
  annotate(
    ...
    x = as_date(phase2_start_25),
    y = 25,
    label = paste0("Start final phase\n(",as_date(phase2_start_25),")"), hjust = "left") +
  theme_minimal() +
  theme(
...
teams_country = FileAttachment('../data/teams_country@18.csv').csv({ typed: true })
...
```{r teams-bar-sss, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, eval=TRUE}
library(countrycode)
library(scales)
