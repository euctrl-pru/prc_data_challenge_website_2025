...
ccc <- teams_valid |> 
  ...
  write_csv(here::here("data", "teams_country@17.csv"))
...
```{r cumulative-plot-rrr, echo=FALSE, warning=FALSE, error=TRUE, message=FALSE}
...
teams_country = FileAttachment('../data/teams_country@17.csv').csv({ typed: true })
...
```{r teams-bar-rrr, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, eval=TRUE}
library(countrycode)
library(scales)
