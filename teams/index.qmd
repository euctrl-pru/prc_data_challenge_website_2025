...
#   # sort-ui: false
#   filter-ui: [image, title, categories, date]
#   # image-align: left
#   # image-height: 1.75in
...
ccc <- teams_valid |> 
  ...
  write_csv(here::here("data", "teams_country@14.csv"))
...
```{r cumulative-plot-ooo, echo=FALSE, warning=FALSE, error=TRUE, message=FALSE}
...
teams_country = FileAttachment('../data/teams_country@14.csv').csv({ typed: true })
...
```{r teams-bar-ooo, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, eval=TRUE}
library(countrycode)
library(scales)
